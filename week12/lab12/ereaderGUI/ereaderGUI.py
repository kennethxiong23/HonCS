...
def getWinSize(width, height, win):
    ...
    smallButton = Button(rectButtonS, "375 x 400", win) 
    medButton = Button(rectButtonM, "450 x 512", win)
    largeButton = Button(rectButtonL, "550 x 640", win)
    ...
    size = None
    while size == None:
        if smallButton.onClick([375, 400]) ==  [375,400]:
            size = [375,400]
        elif medButton.onClick([450,512]) ==  [450,512]:
            size = [450,512]
        elif largeButton.onClick([550, 640]) == [550, 640]:
            size = [550, 640]
    win.close()
    return size
...
def saveUser(swindle):
    ...
    outfile.write("%s,%i,%i\n" %(owner, win.getWidth(), win.getHeight()))
    for book in books:
        book = str(book)
        book = book.strip("[]")
        book = book.replace("'", "")
...
def loadUser():
    ...
    for line in infile:
        if lineCount == 1:
            metadata = line.strip() #gets the owner
    ...
    win = GraphWin("%s's Swindle" %owner, width, height)
    userSwindle = Swindle(owner, ownedBooks, availableBooks, win)
    infile.close()
    return [userSwindle, win]
...
def main():
    if isfile("user-settings.txt"): #checks if it is first time using 
        userData = loadUser()
        userSwindle = userData[0]
        ereaderWin = userData[1]
    else:
